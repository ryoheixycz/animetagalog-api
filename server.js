...
initializeDataFile(SCHEDULED_ANIME_FILE, []); // New file for scheduled anime
...
app.get('/api/anime/:id', async (req, res) => {
  ...
  try {
    ...
    if (animeCache.has(animeId)) {
      ...
      // Check if it has a scheduled date in our custom list
      const customAnimeList = readData(CUSTOM_ANIME_FILE);
      const customAnime = customAnimeList.find(a => a.id === animeId);
      
      if (customAnime && customAnime.scheduleDate) {
        cachedAnime.scheduleDate = customAnime.scheduleDate;
      }
    ...
    try {
      ...
      if (customAnime && customAnime.scheduleDate) {
        animeDetail.scheduleDate = customAnime.scheduleDate;
      }
...
app.get('/api/schedule', (req, res) => {
  ...
  // Filter episodes that have a releaseDate in the future
...
app.post('/api/anime', async (req, res) => {
  try {
    const { anilistId, scheduleDate } = req.body;
    ...
    // Create entry for custom list with optional schedule date
...
app.put('/api/anime/:id/schedule', (req, res) => {
  const animeId = req.params.id;
  ...
  customAnimeList[animeIndex].scheduleDate = scheduleDate;
  
  if (writeData(CUSTOM_ANIME_FILE, customAnimeList)) {
    // Update cache if exists
    if (animeCache.has(animeId)) {
      const cachedAnime = animeCache.get(animeId);
      cachedAnime.scheduleDate = scheduleDate;
      animeCache.set(animeId, cachedAnime);
    }
    
    res.json({ message: 'Schedule updated successfully', scheduleDate });
  } else {
    res.status(500).json({ error: 'Failed to update schedule' });
  }
});

// Add new episode - with support for release dates
app.post('/api/episodes', async (req, res) => {
  ...
  const newEpisode = {
    ...
    title: episodeTitle,
    number: parseInt(nextEpisodeNumber),
